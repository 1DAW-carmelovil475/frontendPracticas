...
export default function AnalisisSection({ active, savedMenus, deleteMenu, setShowAnalysis }) {
  ...
  return (
    <section className={`content-section${active ? ' active' : ''}`}>
      {selectedMenu && (
        <SavedMenuModal menu={selectedMenu} onClose={() => setSelectedMenu(null)} />
      )}
            ...
            >
              <div className="saved-menu-header">
                ...
                <div className="saved-menu-meta">
                  ...
                  {m.restaurantName && (
                    <span style={{ fontSize: '0.78rem', color: 'var(--text-secondary)' }}>{m.restaurantName}</span>
                  )}
                ...
                <div className="saved-menu-actions">
                  ...
                  <button
                    className="btn-fav-action danger"
                    onClick={e => { e.stopPropagation(); deleteMenu(idx) }}
                    title="Eliminar"
                  >
                    <IC.Trash />
                  </button>
